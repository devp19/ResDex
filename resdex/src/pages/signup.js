...
const Signup = () => {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      const displayName = user.displayName ? user.displayName.toLowerCase().replace(/\s+/g, "") : user.email.split("@")[0];
  ...
  const handleGoogleSignupComplete = async (userData) => {
    ...
    try {
      ...
      const usernameQuery = query(collection(db, "usernames"), where("username", "==", userData.displayName));
